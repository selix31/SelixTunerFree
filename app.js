...
// Variables pour le son “bing”
let audioCtx = new (window.AudioContext || window.webkitAudioContext)();
let stableCount = 0; // compteur pour confirmer stabilité
const requiredStable = 5; // nombre de frames consécutives
let lastCorrect = false; // mémorise si le son a déjà été joué

// Affiche les notes à l'écran
...
function playCorrectTone() {
  ...
  oscillator.start();
  oscillator.stop(audioCtx.currentTime + 0.15); // dure 150ms
}
...
function update() {
  ...
  if (freq !== -1) {
    ...
    // Détection de stabilité pour “bing”
    if (Math.abs(percent) < 0.02) {
      ...
      stableCount = 0; // reset si note dévie
      lastCorrect = false;
      if (percent < 0) {
